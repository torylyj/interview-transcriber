...
- **全新机器首次**（换电脑）：本地模式额外加 ①pip 装依赖 ~1–2 分钟（国内镜像）+ ②SenseVoice 模型下载 ~1–3 分钟（魔搭国内直连），即首次 **6–12 分钟**，之后回到 4–8 分钟；云端模式永远不需要下载模型。
...
2. **说话人识别强依赖联网 + DashScope API Key**：本地 ASR 本身能离线，但「采访者/受访人」角色切分是调 qwen-plus 做的，没网/没 Key 只能拿到 `SPEAKER_00/01` 原始标签。
