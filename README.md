...
| 版本 | 日期 | 内容 |
|------|------|------|
| v1.13.3 | 2026-09-07 | **快速档一键安装 + GPU 自动识别**：新增 `scripts/setup_gguf_runtime.py`——nvidia-smi 检测 GPU/驱动自动选包（N 卡 + 驱动 ≥580 → CUDA 版；RTX 50 系 → Blackwell sm_120 包；否则 CPU AVX2），国内多镜像下载（模型：魔搭直连→HF→hf-mirror；运行时：GitHub→ghfast.top→gh-proxy→gh.llkk.cc），SHA-256 校验（模型 2 个 + 运行时 10 平台全量内置），幂等跳过已装组件，支持 --verify/--force/--backend。`transcribe_gguf.py` `--backend` 默认改 **auto**：N 卡 + runtime-cuda 在场自动走 GPU，否则 CPU；修复 auto 后端误传给 exe 的 bug；转录工具标注按实际后端输出「纯 CPU / CUDA GPU 加速」。实测：auto→CPU 与 cuda 两条链路 199s 音频均转录成功 |
