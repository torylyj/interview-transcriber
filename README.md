...
选择方式后，Agent 会根据所选模型的时长能力**自动决定是否切段**（云端超过 5 分钟必须切，本地长音频自动切、短音频整段，均不打断你确认），然后继续自动完成：**转录 → 说话人识别 → 生成摘要与人物信息 → 输出文档 → 自检精简语气词**，最后主动询问你要把文档发到哪里。
...
flowchart TD
    ...
    class S3A cloud;
    class S3B local;
    class S1v,S1a,S2,S25,S26,S35,S36,S37,S4,S6,SEG,NSEG main;
