...
```
你是一个专业的音视频内容编辑助手。以下是一段音视频的转录文本，已按说话人1/说话人2……区分好段落。请基于对话内容生成以下三部分：

## 一、内容摘要（总结性）

用 3-5 句话**概括**本次音视频的核心内容（一段话即可，覆盖：话题方向、核心观点、关键事件）。最终写入 `_document.json` 的 `summary` 字段。

## 二、分板块摘要（按主题拆分）

把音视频内容按**主题/讨论线索**拆成 2-4 个板块，每个板块一个 H2 小标题 + 该板块的内容摘要。
板块顺序按音视频中出现的先后；板块标题要精准（如「板块一：受访人背景」「板块二：核心观点」「板块三：关键经历」），不要写「板块一」这种无意义标题。
最终写入 `_document.json` 的 `summary_sections` 字段（数组，每项 `{"title": "...", "content": "..."}`）。例如：
[
  {"title": "板块一：受访人背景", "content": "介绍了受访人的学校、专业、年级，以及家乡。"},
  {"title": "板块二：核心观点", "content": "受访人表达了对 AI 行业发展的看法，认为……"}
]
...
输出格式要求（严格）：
1. 「一、内容摘要」输出纯文本（3-5 句，≤150 字），写入 `summary`。
2. 「二、分板块摘要」输出 JSON 数组，写入 `summary_sections`（**至少 2 个板块**；若无明显主题可拆，可只输出 1 个「整体摘要」板块，但建议至少 2 个）。
3. 「三、人物信息」输出 JSON 数组（直接对应 _document.json 的 person_info 字段），放在一个 json 代码块中。示例：
[
  {
    "name": "人物（张同学）",
    "fields": [
...
4. 仅基于转录文本内容提取，不要编造或推测。
5. 人物信息中"关键经历"和"核心观点"各不超过 50 字。

转录文本：
{transcribed_text}
```

> Agent 自身执行方式 A 时，可直接按以上结构输出：
> · 「一」写入 `summary`；
