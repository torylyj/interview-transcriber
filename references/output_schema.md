...
```json
{
  ...
  "person_info": [
    {
      "name": "受访人（张同学）",   // 表格标题；单人可写"受访人"或留空
      "fields": [
        {"field": "学校/单位", "value": "..."},
        {"field": "专业/学院", "value": "..."},
        {"field": "年级/身份", "value": "..."},
...
- `person_info` 为空数组 `[]` 或缺失时，**整段「受访人信息」板块自动省略**（采访未透露任何个人信息时不展示空白表格）。
- `person_info` 含多个对象时，**每位受访人渲染为独立表格**（标题取各自 `name`）；单人则只渲染一个表格。
...
👤 受访人信息（无信息则该板块整段省略；多人时每位受访人一个独立表格）
