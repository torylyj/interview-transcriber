...
### 坑 2.3：说话人分离不能靠启发式
- **现象**：SenseVoice 全部标 `SPEAKER_00`，无说话人分离。用关键词/段落长度启发式初分，把受访人长独白里的「好/哪里/补充」等误判为采访者（实测 20+ 处），严重失真。
- ⚠️ **硬性规则**：说话人识别**必须 LLM 语义复核（方式 A，Agent 自身即 LLM，无需 API Key）**。`build_document.py --auto-speakers` 的启发式输出**仅作初分，绝不可直接作为最终交付**；Agent 必须 `--review` 后修正 `conversation[].speaker`。云端同理：Qwen3-ASR-Flash 也不直接分离说话人，「转录 + LLM 语义分段」是统一方案。
