...
### 坑 4.4：同音字校对（Step 3.55）的边界
- **现象**：LLM 同音字校对可能**过度替换**（如「林黛玉」→「林戴玉」、「五四运动」→「无事运动」）或**漏替换**（短语气词 / 跨多句的指代易漏）。
- **对策**：
  - `correct_homophones.py` 用 `context` 精确匹配替换，**无 context 全段替换会被跳过**（避免误伤）；context 不匹配时在 stderr 提示「可能 LLM 幻觉」，让 Agent 复查。
  - 数字/人名/专名按同音字处理时，LLM 必须核对上下文是否合理；Agent 应用 corrections.json 前**人工复核**关键人名/数字。
  - 短语气词（啊/嗯/哦）的误识别通常不是同音字错误（是删除冗余），应放到 Step 3.7（语气词精简）处理，**不要混在 Step 3.55**。
  - 同音字校对只改**字**，不改标点 / 段结构 / 说话人 / 时间码——`build_document.py` 的自动消费只替换 `text`，保留所有 metadata。
