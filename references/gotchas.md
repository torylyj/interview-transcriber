...
- ⚠️ **硬性规则**：
  - **本地**：`transcribe_local.py` 已内置 CAM++，说话人 id 由模型产出，**不再需要 LLM 做说话人切分**；`build_document.py` 只做轻量「角色命名」（`--auto` 按说话人聚合提问特征自动判采访者，`--apply` 可用 `{speaker_roles}` 覆盖），**绝不可再用逐句启发式判定说话人**。
...
- ⚠️ **已知边界（务必诚实告知用户）**：CAM++ 声纹聚类在极嘈杂街头/重叠说话场景下，偶发把同一人误判为两人、或把两人并成一人。出现时用 `build_document.py --apply corrections.json`（`{"speaker_roles": {"SPEAKER_XX": "角色"}}`）一键纠正即可；**不可**为「追求 100% 准确」回退到旧的 LLM 逐句切分（旧法 20+ 误判、且慢）——CAM++ 已是本地最优解，个别误差靠 `--apply` 兜底。

### 坑 2.4：长轮次整块输出难读
- **现象**：每个说话人是一整大块（受访人首段 1275 字挤一团），无段落换行。
