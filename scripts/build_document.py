...
为什么需要它（2026-07-13 复盘）：
  - 之前在临时脚本里手写切分，把 SenseVoice 的 <|withitn|> 当"段间分隔"切，
    ...
    只在「命名」这步做轻量处理：auto_label_roles 按说话人聚合提问
    特征自动判定采访者，--apply corrections.json 可覆盖角色名。

用法：
  # 1) 先看解析结果 + 说话人分布，供 Agent 复核角色命名
  python build_document.py transcript.json --review

  # 2) 自动按说话人聚合判定角色并写出 document.json（summary/person_info 留空待 Agent 填）
  python build_document.py transcript.json document.json --config transcribe_config.json --auto

  # 2.5) Agent 复核角色命名后，把判定结果落盘为最终 document.json：
  #      准备 corrections.json：{"speaker_roles": {"SPEAKER_00":"采访者","SPEAKER_01":"受访人"},
  #                                  "summary":"...", "person_info":[...]}
  #      再应用（按 speaker id 映射角色，自动合并连续同角色为 turn，写出 document.json）：
  python build_document.py transcript.json document.json --apply corrections.json

  # 3) 作为库被 Agent 调用：parse_sentences() / auto_label_roles() / assemble_document()
"""
...
# ── 角色命名（轻量，分离已由 CAM++ 完成） ─────────────────────
# CAM++ 只给出「谁在何时说」（SPEAKER_XX），不区分采访者/受访人。
# 这一步仅做「命名」：按说话人聚合提问特征，自动判定采访者。
_INTERVIEWER_MARKS = [
...
def auto_label_roles(sentences: list) -> dict:
    """按说话人聚合提问/插话特征，自动判定采访者。返回 {speaker_id: role}。

    与旧版「逐句启发式」不同——此处是「按说话人聚合」的统计判定，
    对单句口误/短应答不敏感，可靠得多。
    """
    from collections import defaultdict
    stats = defaultdict(lambda: {"q": 0, "mark": 0, "n": 0})
    for s in sentences:
    ...
    if len(speakers) == 1:
        return {speakers[0]: "受访人"}

    def _score(sp):
        d = stats[sp]
        return d["q"] + d["mark"] * 0.5

    interviewer = max(speakers, key=_score)
    roles = {}
    other_idx = 0
    for sp in speakers:
        if sp == interviewer:
            ...
            other_idx += 1
            roles[sp] = "受访人" if len(speakers) == 2 else f"受访人{other_idx}"
    return roles


def apply_role_labels(turns: list, role_map: dict) -> list:
    """把 turn 的 speaker（SPEAKER_XX）按 role_map 重命名为角色名。"""
    for t in turns:
...
def main():
    ...
    parser.add_argument("--apply", default=None,
                        help="应用 corrections.json（{speaker_roles:{SPEAKER_XX:角色}, summary, person_info}）写出最终 document.json")
    ...
    role_map = auto_label_roles(sentences)
    if args.apply and args.output:
        try:
            ...
            sys.exit(1)
        spk_map = corr.get("speaker_roles", {}) or {}
        if spk_map:
            role_map = {**role_map, **spk_map}  # 用户覆盖优先
        summary = corr.get("summary", "") if isinstance(corr, dict) else ""
        person_info = corr.get("person_info", []) or [] if isinstance(corr, dict) else []
        turns = group_turns(sentences)
        turns = apply_role_labels(turns, role_map)
        doc = assemble_document(data, turns, summary=summary, person_info=person_info)
        with open(args.output, "w", encoding="utf-8") as f:
            json.dump(doc, f, ensure_ascii=False, indent=2)
        print(f"\n✅ 最终 document.json 已写出：{args.output}")
        print(f"   角色命名: {role_map}")
        print(f"   共 {len(turns)} 个对话轮次；summary/person_info 已填入（无则空）")
        return
    ...
    if args.auto and args.output:
        ...
        with open(args.output, "w", encoding="utf-8") as f:
            json.dump(doc, f, ensure_ascii=False, indent=2)
        print(f"\n✅ document.json 已写出（自动角色命名，summary/person_info 待 Agent 填）: {args.output}")
        print(f"   角色命名: {role_map}")
        print(f"   共 {len(turns)} 个对话轮次")
        print("   ⚠️ 若自动命名有误，用 --apply corrections.json 覆盖 speaker_roles 即可。")
        return
    ...
    print("  python build_document.py transcript.json out.json --apply corrections.json   # 覆盖命名+填 summary/person_info")
