...
将采访内容（视频或音频）全流程处理为带说话人识别的转录文档：输入预处理（音频跳过转 MP3、无静帧）-> 选转录方式 -> 模型按能力自动决定是否切段 -> 转录（云端/本地可选）-> 说话人识别 -> 生成摘要与人物信息 -> 生成 Markdown -> 自检精简语气词 -> 可选分发到在线文档平台。全部处理完成后主动询问用户要将文档发送到哪里。
...
- **标题**：文档标题（无冗余装饰）
- **人物静帧**（仅视频输入）：居中显示，280px 宽度（本地路径引用）；音频输入时无此行
...
```bash
rm -f _seg*.mp3 _upload.md *_raw.txt *segments.json transcribe_config.json 输出.mp3
```
...
所有文档处理（转录、说话人识别、摘要与人物信息、Markdown 生成、临时文件清理）**全部完成后**，必须主动询问用户希望将整理好的文档发送到哪里，再执行分发。不要默默结束对话，也不要未经确认就擅自上传到任何外部平台。
...
- **本地文件** — `<标题>.md` 已生成在 output_dir，直接告知路径即可交付
