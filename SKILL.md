...
description: |
  街采视频（街头采访）全流程处理技能。覆盖：从视频文件提取人物静帧 -> 视频转 MP3 音频 -> 云端转录（Qwen3-ASR-Flash，推荐）或本地转录（faster-whisper + pyannote.audio 声纹分离，质量较差）-> Qwen-Plus LLM 智能说话人识别（区分采访者/受访人） -> Qwen-Plus LLM 生成内容摘要与受访人人物信息（置于文档正文最前面） -> 生成 Markdown 文档（第一行嵌入静帧） -> 可选输出到钉钉在线文档/本地文件等。
  This skill should be used when the user asks to transcribe street interview / interview videos, convert video to text with speaker diarization, or process video files from directories like "H:\\街头采访".
agent_created: true
---
...
将街头采访视频全流程处理为带说话人识别的转录文档：提取静帧 -> 音频转换 -> 转录（云端/本地可选）-> 说话人识别 -> 生成摘要与人物信息 -> 生成 Markdown -> 可选分发到钉钉等平台。
...
- 街采场景通常 2 人（采访者+受访人），脚本默认 `min_speakers=2, max_speakers=2`
...
调用 `dashscope.Generation.call(model='qwen-plus')` 对转录文本做语义分析：

```python
import dashscope
dashscope.api_key = API_KEY

prompt = """You are a professional transcript editor. Below is a raw transcription of a street interview video. The text contains dialogue from two speakers: the interviewer (采访者) and the interviewee (受访人), but they are mixed together in one continuous block.
...
```python
prompt = """以下是一段街头采访的转录文本，已通过声纹分离区分出两个说话人（SPEAKER_00 和 SPEAKER_01）。
...
- 云端模式：`Qwen-Plus LLM 语义分析`
- 本地模式：`pyannote.audio 声纹分离 + Qwen-Plus LLM 角色映射`

**长文本处理：** Qwen-Plus 单次输入建议不超过 8000 字符，超过时分段处理后拼接。
...
调用 `dashscope.Generation.call(model='qwen-plus')` 分析已分好说话人的转录文本：

```python
import dashscope
dashscope.api_key = API_KEY

prompt = """你是一个专业的采访内容编辑助手。以下是一段街头采访的转录文本，已按采访者和受访人区分好段落。请基于对话内容生成以下两部分：
...
**内容上限处理：** 钉钉 doc create 限制内容不超过 10000 字符。超长文档需分段：
1. 将完整内容按 8000 字符分割（留余量），在 `采访者` / `受访人` 标签边界切分，保持段落完整
2. `dws doc create` 上传第一段
3. `dws doc update --node <node_id> --mode append --content-file <part2.md>` 追加后续段落
...
**钉钉特有注意事项：**
- 只保留一个在线文档，不要额外上传 MD/MP3 文件到钉钉
- **dws 命令必须通过 bash 直接执行**：Python `subprocess` 调用 `dws.cmd` 传递中文参数（如文档名称）会触发 `TOKEN_VERIFIED_FAILED` 错误。正确做法：用 Python 生成内容文件（.md），然后用 Bash 工具直接执行 `dws` 命令
- **并发 dws API 调用会触发限流**：多个 `dws doc create` / `doc media insert` 并行执行时会出现 `TOKEN_VERIFIED_FAILED`。必须串行执行，操作间间隔 5 秒
- `dws doc read --format markdown` 的 JSON 输出中 text 字段含 `\uXXXX` 转义，`json.loads` 可正常处理
- `dws doc create` 输出 JSON 中 `nodeId` 字段即为文档标识，后续 `doc update` 和 `doc media insert` 使用 `--node <nodeId>`
...
如用户需要输出到其他平台（飞书文档、腾讯文档、Notion 等），参照对应平台的 API/CLI 工具进行上传。核心交付物为 Step 3.6 生成的 Markdown 文件（含摘要、人物信息、对话正文）。
...
2. ✅ **云端模式：Qwen-Plus LLM 语义分析**：Qwen3-ASR-Flash 转录输出连续文本，LLM 理解对话内容智能切分，准确率 95%+
3. ✅ **本地模式：pyannote.audio 声纹分离 + Qwen-Plus LLM 角色映射**：pyannote 基于声纹区分说话人，LLM 将 SPEAKER 标签映射为采访者/受访人角色

Qwen3-ASR-Flash 不直接支持说话人分离，fun-asr 虽支持但需 OSS 文件上传（file:// 本地路径不可用）。云端最优方案：Qwen3-ASR-Flash 转录 + Qwen-Plus 语义分段。本地备选方案：faster-whisper 转录 + pyannote.audio 声纹分离 + Qwen-Plus 角色映射。
...
- **摘要与人物信息必须生成**：说话人识别完成后必须执行 Step 3.6，调用 Qwen-Plus 生成内容摘要和受访人人物信息，插入到文档正文最前面（静帧之后、元数据之前），让读者快速了解采访核心内容和受访人背景
...
- **本地声纹分离**：使用 pyannote.audio（`pyannote/speaker-diarization-3.1`），需 HuggingFace Token + 接受模型条款，街采场景固定 2 人
...
- **长文本 LLM 分段**：Qwen-Plus 单次输入建议不超过 8000 字符，超过时分段处理后拼接
